...
# JWT Token配置
JWT_CONFIG = {
    'secret_key': 'your-secret-key-here-change-in-production',  # JWT签名密钥
    'algorithm': 'HS256',  # JWT算法
    'expires_in': 3600,    # Token过期时间（秒）
    'refresh_expires_in': 86400  # 刷新token过期时间（秒）
}
...
DATA_CONFIG = {
    ...
    'database_dir': os.path.join(BASE_DIR, 'data', 'database')        # 数据库存储目录
}